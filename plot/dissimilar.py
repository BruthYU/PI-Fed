...
if args.plot_type == 'latest':
    ...
    task_y_lim = {'cifar100_10':(0.58, 0.80), 'cifar100_20':(0.58, 0.80),
                  'tinyimagenet_10':(0.58, 0.80), 'tinyimagenet_20':(0.58, 0.80)}
...
for t_idx, t_name in enumerate(task_names):
    ...
    for m_idx, m_name in enumerate(method_names):
        ...
        axs[plot_count].plot(t, ys, label = label,linewidth=1.6,color=colors[m_idx])
    ...
    axs[plot_count].yaxis.set_major_locator(MultipleLocator(0.2))
    axs[plot_count].set_xlabel('Task Learned', fontweight='bold', family = 'serif')
    axs[plot_count].set_ylabel('Latest Acc.',fontweight='bold',family = 'serif')
